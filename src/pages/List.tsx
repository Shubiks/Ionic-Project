...
const List: React.FC = () => {
    ...
    const [showAlert, setShowAlert] = useIonAlert();
    const  [showToast] = useIonToast();
    const [selectedUser, setSelectedUser] = React.useState<any>(null);
    const modal= useRef<HTMLIonModalElement>(null);
    const cardModal= useRef<HTMLIonModalElement>(null);
    const [presentingElement, setPresentingElement] = React.useState<HTMLElement | null>(null);
    const page = useRef(null);

    const [activeSegment, setActiveSegment] = React.useState<any>('details');
    ...
    const clearList = () => {
        showAlert({
            header: 'Confirm!',
            message: 'Are you sure you want to clear all users?',
            buttons:[
            ...
            </IonRefresher>
                {
                    loading &&(
                        [...Array(10)].map((_, index) => (
                            ...
                            <IonModal ref={modal}  breakpoints={[0,0.5,0.8]} initialBreakpoint={0.5} 
